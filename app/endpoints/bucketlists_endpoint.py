...
class BucketListCrud(Resource):
    ...
    def post(self, user, *arg, **kwargs):
        ...
        if res:
            bucketlist = {"id": res.id, "name": res.name,
                          "description": res.description}
            data = {"status": "success",
                    "message": "Bucketlist added successfully", "data": bucketlist}
            return data, 201
        else:
            data = {"status": "failed",
        ...
        if bucketlist_id:
            ...
            if bucketlists:
                ...
                if bucketlists.has_next:
                    results["next"] = "/api/v1/bucketlists?page={}&pagesize={}".format(
                        bucketlists.next_num, items_per_page)

                if bucketlists.has_prev:
                    results["prev"] = "/api/v1/bucketlists?page={}&pagesize={}".format(
                        bucketlists.prev_num, items_per_page)

                results["current"] = "/api/v1/bucketlists?page={}&pagesize={}".format(
    ...
    def put(self, user, bucketlist_id=None, *arg, **kwargs):
        ...
        if bucketlist_id:
            ...
            if bucketlist:
                ...
                bucketlist.save()
                data = {"status": "success", "message": "Bucketlist updated successfully", "data": {
                    "id": bucketlist.id, "name": bucketlist.name, "description": bucketlist.description}}
                return data, 201
            else:
                data = {"status": "failed",
    ...
    def delete(self, user, bucketlist_id=None, *arg, **kwargs):
        ...
        if bucketlist_id:
            ...
            if bucketlist:
                ...
                data = {"status": "failed",
                        "message": "Bucketlist failed to delete"}
                return data, 200
        else:
            data = {"status": "failed", "message": "No bucketlist provided"}
            return data, 404
